...
Player::Player() : _position(0, 0), _direction(Up), _wantedDirection(Up), _map(nullptr), _size(6)
{
    CreateBody();
    return;
}

Player::Player(Coord position, Map *map) : _position(position), _direction(Up), _wantedDirection(Up), _map(map), _size(6)
{
    CreateBody();
    return;
}

Player::Player(int x, int y, Map *map) : _position(x, y), _direction(Up), _wantedDirection(Up), _map(map), _size(6)
{
    CreateBody();
    return;
}
